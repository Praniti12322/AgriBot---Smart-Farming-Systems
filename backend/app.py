...
app = FastAPI(title="AgriBot - Smart Farming Systems")
...
# ── Agriculture Assistant (Krishi AI) ─────────────────────────────
...
def assistant_chat(request: AssistantChatRequest, current_user: str = Depends(get_current_user)):
...
"""
        model_name = os.environ.get("MODEL", "llama-3.1-8b-instant")

    try:
        from .quiz_logic import client # Use the shared Groq client
        ...
        return {"response": cleaned_content, "urgency": detected_urgency}
    except Exception as e:
        return {"response": f"Error: {str(e)}", "urgency": "Low"}
